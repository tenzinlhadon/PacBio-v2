...
# run_post_fastqc= eval(data_parsed["postfastqc"])
f.close()
...
# Changing the CWD
os.chdir('/import/ContaVect_hm_coordinates/')
...
try:
	...
	from pyDNA.Blast import BlastHit
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
	...
	def __call__(self):
		...
		* Short read alignment with /tools/bwa.kit/bwa mem
	...
	def _iterative_masker (self): #### TODO The fuction directly manipulate reference field= change that
		...
		hit_list_all = []
		df = pd.DataFrame()
		count_ref = Reference.countInstances()
        
		print(count_ref)
		for i in range(Reference.countInstances()-1, -1, -1):

			# Extract subject and query_list from ref_list
			subject = Reference.Instances[i]
			query_list = Reference.Instances[0:i] + Reference.Instances[i+1:count_ref+1] 
			...
			for element in hit_list:
				...
				#df = df.append(report_hitobject , ignore_index=True)
